// Sensor weight: the Radar-owned map from array geometry to a path weight.
//
// This is the Phase-6 native owner of what `solvers/common.py` computes in
...
//
//   VIA row (row_kind = 0), an interaction at a site:
//     d_in  = site_in - tx      a = max(|d_in|, 1e-6)   u_in  = d_in / a
//     d_out = rx - site_out     b = max(|d_out|, 1e-6)  u_out = d_out / b
//     L        = a + fixed_length_m + b
//     tau_rate = ( u_in . (v_site - v_tx) + u_out . (v_rx - v_site) ) / c0
//     pattern directions: v_t = site_in - tx,  v_r = site_out - rx
//
//   DIRECT row (row_kind = 1), transmitter straight to receiver:
//     d_in = rx - tx            a = max(|d_in|, 1e-6)   u_in = d_in / a
//     L        = a
//     tau_rate = u_in . (v_rx - v_tx) / c0
//     pattern directions: v_t = rx - tx,  v_r = tx - rx
//
//   tau_rt = L / c0
//
//   G_t = pattern(local(v_t)),  G_r = pattern(local(v_r))     [POWER gains]
//   pattern_gain = G_t * G_r                                  [published]
//
//   scale = sqrt(max(intensity, 0)) * sqrt(max(G_t * G_r, 0))
//           * ( spreading_mode           ? wavelength / (4 pi max(L, 1e-6)) : 1 )
//           * ( tx_power_mode            ? tx_amplitude                     : 1 )
//           * ( legacy_real_polarization ? pol                              : 1 )
//
//   out = weight_in * scale                                   [complex, real scale]
//
// THE POINT OF THE THREE MODE FLAGS. They are driven directly by the batch's
// provenance booleans, which is how the single-count rule becomes a kernel
// argument rather than a comment. A Channel-sourced weight already carries
// wavelength/(4 pi d) per leg and sqrt(tx_power) from the source endpoint's
// powers_w, so it arrives with spreading_mode = 0 and tx_power_mode = 0 and
// CANNOT have either applied a second time. The legacy real-amplitude route
// carries neither, passes 1 for both, and reproduces the Torch expression it
// replaces bit for bit. There is no third setting and no default: the flag is
// what the provenance says it is.
//
// `pol` is the LEGACY receive projection and it exists behind
// legacy_real_polarization for one reason. Channel already applies the endpoint
// polarization projection - its published contract is
// "world_cartesian_complex3_then_receiver_projection" - so a Radar-side
// projection on a Channel-sourced weight is a SECOND projection. It survives
// only for the real-amplitude route, which has no Jones transport to project
// with. Its sign is physics rather than bookkeeping: with reflection_flip the
// transmit polarization is mirrored about the surface normal first, and the
// mirrored vector can point away from the receive polarization, so `pol` is a
// SIGNED real factor and a mirrored row is exactly -1 times the unmirrored one.
//
//   n = normalize(normal)
//   t = normalize(pol_tx[ti]);  if reflection_flip: t = t - 2 (t . n) n
//   t = normalize(t)
//   pol = t . normalize(pol_rx[ri])
//
...
// weight. The velocities, the normals, the polarization vectors, the local
// frame, and the pattern table are constants. `tau_rate` depends on the
...
__device__ __forceinline__ RowTerm evaluate_row(const RowInputs& in, int64_t k) {
  ...
  const Vec3 axis_right = load3(in.local_axes, 0);
  const Vec3 axis_up = load3(in.local_axes, 1);
  const Vec3 axis_back = load3(in.local_axes, 2);
  ...
  const float clamped_length = length < kMinDistance ? kMinDistance : length;
  const float spread = in.spreading_mode != 0
      ? in.wavelength_m / (4.0f * kPiF * clamped_length)
      : 1.0f;
  const float tx_gain = in.tx_power_mode != 0 ? in.tx_amplitude : 1.0f;

  float polarization = 1.0f;
  if (in.legacy_real_polarization != 0) {
    const Vec3 normal = normalize3(load3(in.normals, k));
    Vec3 t = normalize3(load3(in.pol_tx, ti));
    if (in.reflection_flip != 0) {
      t = sub3(t, scale3(normal, 2.0f * dot3(t, normal)));
    }
    t = normalize3(t);
    polarization = dot3(t, normalize3(load3(in.pol_rx, ri)));
  }

  const float scale =
      intensity_amplitude * gain_amplitude * spread * tx_gain * polarization;
  term.scale = scale;
  ...
  if (raw_intensity > 0.0f) {
    term.dscale_dintensity =
        0.5f / intensity_amplitude * gain_amplitude * spread * tx_gain * polarization;
  }

  // d(scale)/d(direction), through sqrt(clamp(G_t G_r, 0)).
  if (gain_product > 0.0f) {
    const float common =
        0.5f / gain_amplitude * intensity_amplitude * spread * tx_gain * polarization;
  ...
  // d(scale)/d(length), through the spreading term only. The clamp's flat
  // region has zero derivative, like every other clamp in this file.
  float dscale_dlength = 0.0f;
  if (in.spreading_mode != 0 && length > kMinDistance) {
    dscale_dlength = -scale / clamped_length;
  }
...
struct GeometryTensors {
  ...
  const torch::stable::Tensor& normals;
  const torch::stable::Tensor& pol_tx;
  const torch::stable::Tensor& pol_rx;
  const torch::stable::Tensor& local_axes;
...
RowInputs build_inputs(
    ...
    double wavelength_m,
    double tx_amplitude,
    int64_t spreading_mode,
    int64_t tx_power_mode,
    int64_t legacy_real_polarization,
    int64_t reflection_flip) {
  ...
  check_row_vector(t.normals, num_paths, "normals");
  ...
  check_cuda_float(t.pol_tx, "pol_tx");
  check_cuda_float(t.pol_rx, "pol_rx");
  STD_TORCH_CHECK(
      t.pol_tx.numel() == static_cast<int64_t>(num_tx) * 3,
      "pol_tx must hold one world polarization vector per transmitter.");
  STD_TORCH_CHECK(
      t.pol_rx.numel() == static_cast<int64_t>(num_rx) * 3,
      "pol_rx must hold one world polarization vector per receiver.");
  check_cuda_float(t.local_axes, "local_axes");
  STD_TORCH_CHECK(
      t.local_axes.numel() == 9,
      "local_axes must hold the three world-space local frame axes.");
  ...
  in.normals = t.normals.const_data_ptr<float>();
  in.pol_tx = t.pol_tx.const_data_ptr<float>();
  in.pol_rx = t.pol_rx.const_data_ptr<float>();
  in.local_axes = t.local_axes.const_data_ptr<float>();
  ...
  in.wavelength_m = static_cast<float>(wavelength_m);
  in.tx_amplitude = static_cast<float>(tx_amplitude);
  in.spreading_mode = spreading_mode != 0 ? 1 : 0;
  in.tx_power_mode = tx_power_mode != 0 ? 1 : 0;
  in.legacy_real_polarization = legacy_real_polarization != 0 ? 1 : 0;
  in.reflection_flip = reflection_flip != 0 ? 1 : 0;
  return in;
}
