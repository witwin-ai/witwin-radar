"""The shared Torch pieces every processing stage is built from.

Four things live here: the window family, the pulsed replica and its
correlation, the published convention constants, and the two records every
stage reads - :class:`ProcessingAxes` and :class:`ProcessingCube`.

**Windows are built from ``arange`` and ``cos``, not from
``torch.hamming_window``.** The frozen vendor-DSP primitive list does not
contain the window constructors, and this package is not allowed to grow that
list. The definitions here are PERIODIC: a symmetric window makes the transform
of a pure tone asymmetric about its bin, which is exactly the property an
exact-bin assertion leans on.

**The correlation does not upcast the signal to ``complex128``.** The upcast
would double the transform cost of every pulsed frame to buy precision the
float32 input never had. The REPLICA is still built in float64, because it is a
per-frame constant of a few hundred elements whose accuracy sets the accuracy of
every delay estimate downstream, and it is cast to the signal's dtype only at
the multiply.
"""
...
from ..sensors import SensorArraySpec
...
def real_dtype_of(tensor: torch.Tensor) -> torch.dtype:
    """The real dtype that matches ``tensor``'s precision: float64 for a
    float64 or complex128 tensor, float32 otherwise."""

    return torch.float64 if tensor.dtype in (torch.float64, torch.complex128) else torch.float32
...
def remove_mean(signal: torch.Tensor, *, dim: int) -> torch.Tensor:
    ...
    A named function behind an explicit flag that defaults to off, because it
    is a clutter operation: a component-export test that asks for the clutter
    cube and silently gets the clutter removed from it would be comparing two
...
def pulse_samples(spec: PulsedSpec, device: torch.device | str = "cpu") -> torch.Tensor:
    """The transmitted replica on the ADC grid, as ``complex128[M_p]``.

    ``M_p = round(T_p / T_s)`` samples at ``u = m T_s``, with the analytic
    amplitude ``1 / sqrt(T_p)`` and the analytic phase: the same function the
    pulsed synthesis kernel evaluates at a continuous fractional delay. In
    float64 because it is a per-frame constant of a few hundred elements whose
    accuracy sets the accuracy of every delay estimate downstream.
    ...
    count = int(spec.pulse_sample_count)
    if count < 1:
        raise ValueError(
            f"the pulse spans {count} samples on this grid: it is shorter than "
            "one sample period, so there is no replica to correlate against"
        )
    target = torch.device(device)
    u = torch.arange(count, dtype=torch.float64, device=target) * float(spec.sample_period_s)
    envelope = torch.full((count,), float(spec.pulse_amplitude), dtype=torch.float64, device=target)
    if spec.is_linear_fm:
        cycles = 0.5 * float(spec.bandwidth_hz) * u * u / float(spec.pulse_width_s)
...
#: A positive Doppler bin is a closing target, in every waveform. The FMCW beat
#: cube is the conjugate of Channel's phasor, so its raw slow-time tone sits at
#: ``+f_ref tau_rate`` while the OFDM and pulsed tones sit at
#: ``-f_ref tau_rate``; the two are reconciled EXACTLY ONCE, inside
#: :func:`~witwin.radar.processing.range_doppler.range_doppler_map`, driven by
#: :attr:`ProcessingAxes.doppler_sign`. Nothing else in this package may look
#: at a phasor.
PROCESSING_DOPPLER_CONVENTION = "positive_doppler_bin_is_closing"

#: Every range profile and every Range-Doppler map is an AMPLITUDE estimate,
#: normalised so that an isolated path row peaks at ``|C_rt|`` times the
#: window's coherent gain. The three waveforms' native transforms have three
#: different gains (``N`` for an unnormalised beat FFT, ``1`` for a CIR inverse
#: transform, ``1`` for a unit-energy matched filter), and publishing all three
#: would make a cross-waveform amplitude comparison a per-waveform bookkeeping
#: exercise.
PROCESSING_AMPLITUDE_CONVENTION = "peak = |C_rt| * window_coherent_gain; transforms are amplitude normalised"

#: The name of the slow-time axis per waveform.
...
class ProcessingAxes:
    ...
    The single physical-axis owner for every waveform. It is built from the SI
    waveform SPEC, the ``SensorArraySpec`` and the ``SynthesisResult``'s own
    published conventions, never from the engineering-unit radar configuration:
    a second reader of those units would be a second conversion, and a
    conversion that is wrong once is wrong everywhere.
    ...
    ``doppler_sign`` is DERIVED from the cube's published ``phasor`` - not from
    the waveform's name, and not by editing the phasor constants - and
    :func:`~witwin.radar.processing.range_doppler.range_doppler_map` is the one
    place it is applied.
    """
    ...
    def from_synthesis(
        ...
        spec: FmcwSpec | OfdmSpec | PulsedSpec,
        ...
        *,
        ...
        if kind == "fmcw":
            ...
            replica = pulse_samples(spec, device=device)
...
class ProcessingCube:
    ...
    Nothing here knows about ``row_valid``. A dead row is masked on the WEIGHT
    before the waveform kernel launches, so its contribution to the cube is a
    literal zero and no downstream stage has anything to mask.
    """
    ...
    @classmethod
    def from_synthesis(cls, result: SynthesisResult, axes: ProcessingAxes) -> "ProcessingCube":
        ...
        The transpose from the SINK-MAJOR composed pair rank to the TX-MAJOR
        virtual-antenna index is ``synthesis/assembly.py::assemble_frame_cube``,
        CALLED here rather than reimplemented: two packers for one layout is
        how a TX/RX swap ships silently on a square array.
        """
