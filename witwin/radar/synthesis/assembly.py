...
* :class:`~witwin.radar.paths.RadarPathBatch` is what the two-way
  composer PRODUCED.
...
FMCW_OUTPUT_SPECTRUM = "spectrum"
FMCW_OUTPUT_BEAT = "beat"
FMCW_OUTPUT_DOMAINS = (FMCW_OUTPUT_SPECTRUM, FMCW_OUTPUT_BEAT)
...
class OfdmSpec:
    ...
    **Cyclic prefix.** ``max_expected_delay_s`` is a CONFIGURED bound - the
    ...
    :class:`FmcwSpec` and obey the same rule through the same helper: the
...
class PulsedSpec:
    ...
    (:mod:`witwin.radar.processing.range_doppler`), because synthesis owns the
...
class SynthesisResult:
    """One synthesis product with waveform, phasor, axes, and domain as data."""
    ...
    @classmethod
    def from_ofdm(cls, cube: torch.Tensor, spec: OfdmSpec) -> "SynthesisResult":
        return cls(
            cube=cube,
            kind="ofdm",
            axes=OFDM_AXES,
...
def pair_slot_index(
    *,
    ...
    ``witwin/radar/cuda/fmcw_beat.cu`` already computes:
...
launch. ``synthesize_fmcw`` does exactly that with ``torch.where``, and it
...
* the exported batch shares the SAME
  :class:`~witwin.radar.paths.RadarPathTopology` OBJECT, so "row
