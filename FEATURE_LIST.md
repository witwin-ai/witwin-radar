...
- `Result.sample_times_s` and `Paths.sample_times` publish each frame's observation schedule as a `float64` array rather than a tuple of Python floats. An ADC-refreshed frame schedules one instant per chirp, transmitter and sample, so materialising that as Python objects cost more than synthesizing the frame; a caller that wants floats calls `.tolist()` on the frame it needs. Compare two schedules with `numpy.array_equal`, not `==`.
...
- Two instrument artifacts are declared rather than assumed, and both are off by default. `Leakage(isolation_db, delay_s)` on `Radar` models transmit-to-receive antenna coupling as one more path row, synthesized by the same waveform kernel as the echo, so its near-range tone and the skirt the range transform spreads over the first bins are present for a sub-metre dataset. `Iq(gain_db, phase_deg)` models quadrature demodulator imbalance as `alpha*x + beta*conj(x)`, which mirrors the Doppler axis at the `image_rejection_db` it publishes; it sits before the gain, so the AGC and the quantiser spend dynamic range on the image the way a real receiver does.
