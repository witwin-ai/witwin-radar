...
- `AdaptiveMotionSpec.interpolation_nodes` sets how many sampled instants carry one accepted interval and therefore the polynomial order of the interpolated delay; the default 5 is a quartic and 2 is the former linear rule. The native interpolant takes K nodes with caller-supplied weights that must sum to one and publishes no weight derivative. A higher order cannot lengthen an interval the probe-spacing bound already fixed, so it applies only where that bound is not enforced.
- `max_interval_s` bounds how long a run may go without looking for a path birth and is not an accuracy control. It is not enforced for a family certified complete for all time, which cannot gain or lose a row; `adaptive_diagnostics` carries `max_interval_enforced` per frame. Where it does apply, the run starts from the coarsest partition it allows instead of bisecting down to it.
