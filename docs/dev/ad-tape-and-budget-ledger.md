...
1. **Refusal predicates.** `torch.is_grad_enabled`,
   ...
   never construct a value. `witwin/radar/policy.py` owns these guards and is allowed nothing else. It is asserted
...
2. **Result construction, one expression.** `rcs_amplitude`'s `torch.sqrt`,
   ...
   `torch-orchestration` for exactly that reason. `scattering.py`'s whole
