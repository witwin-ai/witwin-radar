...
| GOV-020 | native compatibility | sensor weight mode flags, `PolarizationSpec`, `from_real_amplitudes` | deleted real-amplitude route remains in Python and native ABI | 1/7 | Channel-sourced parity with retired flags/schemas absent | closed | native manifest has 35 current operators; real-amplitude schemas and sensor mode flags absent |
...
inventory includes three scatter-direction companions, the continuous-time oscillator query,
and three adaptive path interpolation companions (35 operators total, ABI 6).
