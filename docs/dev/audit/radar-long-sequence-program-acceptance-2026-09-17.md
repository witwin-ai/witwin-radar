...
| # | 内容 | 结果 | 提交 |
| --- | --- | --- | --- |
| 1 | 高阶插值 | K 节点 Lagrange 原生插值，默认 5 节点。转子帧 76–82 ms → **19.0 ms**，探针 73–101 → **9** | `85d7010` |
| 2 | 区间上限与二分 | 上限仅在拓扑不可证明完整时强制；强制时从最粗允许分区起步。MIMO 探针 65 → 17 | `b04e9f9` |
| 3 | 跨帧热启动 | **实测无余量**：可回收探针 0/69、0/36、13/23511（0.055%）。不实现 | `9a55025` |
| 4 | 多径发现批处理 | **阻塞在 Channel**：批处理值 4.15×（整帧 2.4×），但需要配对限制或句柄切分 | `27428f5` |
| 5 | 长序列流式出口 | `Radar.stream(...)`，峰值显存与帧数脱钩，逐帧逐位一致 | `ddc084e` |
| 6 | 完整性标志语义 | `path_set_complete` 与 `motion_sampling_exhaustive` 分开发布 | `6723b2a` |
...
MATLAB 对照已于同日复跑，结果见 [2026-09-17 对照报告](radar-matlab-comparison-2026-09-17.md)：
四个基准动态场景上 WiTwin 比 MATLAB 快 1.6～2.9 倍，反转了上一版"慢 21～53 倍"的结论；
4 倍过采样旋转场景仍是 MATLAB 更快（0.75×），其解析参考误差 0.291% 也仍低于 WiTwin 的 0.476%。
没有执行 Linux、发行 wheel 或远程工作流。
