...
def test_every_operator_has_an_owner_a_test_and_a_caller(manifest):
    """Every registered symbol names a production entry point: no orphan binding."""
    ...
    assert caller_free == set(), sorted(caller_free)
...
def test_every_manifested_source_is_a_build_input(manifest):
    ...
    sources = {str(path.relative_to(REPO_ROOT)).replace("\\", "/") for path in runtime.extension_sources()}
...
def test_the_load_check_covers_every_operator_family(manifest):
    ...
    checked_families = {
        entry["family"] for entry in manifest["operators"] if entry["symbol"] in runtime._REQUIRED_OPERATORS
    }
    assert checked_families == families, sorted(families - checked_families)


def test_the_jit_build_directory_is_keyed_by_the_source_set(monkeypatch):
    ...
    fingerprint = runtime.source_fingerprint()
    assert len(fingerprint) == 16
    assert fingerprint in runtime.default_build_directory().name
    # Deterministic: the same sources always key the same directory.
    assert runtime.source_fingerprint() == fingerprint
    ...
    monkeypatch.setattr(runtime, "extension_sources", lambda: [kernel, edited])
    assert runtime.source_fingerprint() != fingerprint
    monkeypatch.undo()
    assert runtime.source_fingerprint() == fingerprint
...
def test_every_load_route_validates_the_required_operators(monkeypatch):
    ...
    with pytest.raises(ImportError, match="stale"):
        runtime._require_operators(pathlib.Path("fake.pyd"))
