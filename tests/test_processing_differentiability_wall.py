...
* ``ca_cfar``: ``d(threshold)/d(power)`` summed to **1.51e4**, and under a
...
class _Scene:
    ...
    def __init__(self, axes, array, cube) -> None:
        ...
        self.detected = ca_cfar(self.power, **DETECTOR)
...
def test_the_no_partial_result_instrument_is_not_vacuous(scene, watch):
    ...
    detections = ca_cfar(scene.power, **DETECTOR)
    assert "avg_pool2d" in watch.calls
    point_cloud(detections, scene.rd, scene.array, max_points=64)
    assert "argwhere" in watch.calls
    assert "topk" in watch.calls


# ---------------------------------------------------------------------------
# ABOVE the wall: the declared-differentiable stages stay live
# ---------------------------------------------------------------------------
...
CFAR_ENTRIES = {"ca_cfar": ca_cfar, "os_cfar": os_cfar}
...
def test_the_detectors_still_detect_and_the_guard_changed_no_value(scene):
    ...
    detached = ca_cfar(scene.power.clone().requires_grad_(True).detach(), **DETECTOR)
...
# ---------------------------------------------------------------------------
# BELOW the wall: the point cloud and its topk
# ---------------------------------------------------------------------------
...
def test_the_point_cloud_refuses_a_gradient_before_any_compute(scene, watch):
    ...
    with pytest.raises(RuntimeError, match="witwin.radar.processing.detection.point_cloud is not differentiable"):
        point_cloud(scene.detected, live, scene.array, max_points=64)
    assert watch.calls == []
...
def test_the_point_cloud_refuses_a_forward_dual_before_any_compute(scene, watch):
    with forward_ad.dual_level():
        ...
        with pytest.raises(RuntimeError, match="a forward tangent"):
            point_cloud(scene.detected, dual, scene.array, max_points=64)
    assert watch.calls == []
...
def test_the_point_cloud_refuses_a_live_detection_threshold(scene, watch):
    ...
    with pytest.raises(RuntimeError, match="detection_threshold carries a gradient"):
        point_cloud(live, scene.rd, scene.array, max_points=64)
    assert watch.calls == []
...
def test_the_point_cloud_still_produces_its_one_point(scene):
    cloud = point_cloud(scene.detected, scene.rd, scene.array, max_points=64)
    assert len(cloud) == 1
...
# ---------------------------------------------------------------------------
# BELOW the wall: the argmax angle estimators
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# BELOW the wall: tracking, the second door
# ---------------------------------------------------------------------------
...
def test_every_wall_refusal_comes_from_the_one_owner():
    ...
    Asserting the single import here is what stops a future stage from growing
    a second voice, and with it a second behaviour: a guard that checks
    ``requires_grad`` only lets a forward dual walk straight through.
    """
