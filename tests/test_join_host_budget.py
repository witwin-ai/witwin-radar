...
def test_a_multipath_frame_costs_exactly_two_host_observations(multipath, monkeypatch):
    ...
    assert copies == counter.host["item"]
...
def test_multipath_costs_no_more_per_frame_than_line_of_sight(monkeypatch):
    ...
    for label, components, depth in (("los", frozenset({"los"}), 0), ("multipath", MULTIPATH, 1)):
        ...
        measured[label] = (composed.path_count, dict(counter.host))
        monkeypatch.undo()
...
def test_the_per_frame_host_budget_is_flat_in_slot_count(slot_spike, monkeypatch):
    ...
    for slots in (1, 8, 64, 256):
        ...
        assert sum(measured[slots].values()) == 2, (slots, measured[slots])
