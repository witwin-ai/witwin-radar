...
from reference.path_math import pattern_gain_from_vectors
from support import multi_endpoint_geometry as geo
from support.pose import local_from_world_vectors
from support.simulate_fixture import LOOK_AT_M, SITE_POSITIONS_M, fixture_targets, static_scene
...
def _simulate(radar: Radar, *, times=(0.0,), **options):
    return radar.simulate(static_scene(), fixture_targets(radar), times=times, **options)


# ---------------------------------------------------------------------------
# The no-op, asserted bitwise
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# A directional pattern, against the independent Torch oracle
# ---------------------------------------------------------------------------
...
def _oracle_amplitude(radar: Radar, result, sites: torch.Tensor, pattern) -> torch.Tensor:
    ...
    gain_tx = pattern_gain_from_vectors(pattern, local_from_world_vectors(radar, tx_target - tx))
    gain_rx = pattern_gain_from_vectors(pattern, local_from_world_vectors(radar, rx_target - rx))
...
# ---------------------------------------------------------------------------
# The direct contract: refusals and the AD checks
# ---------------------------------------------------------------------------
...
def _unit_loss(stage, batch, sites, tx_pos, rx_pos) -> torch.Tensor:
    weight = stage.apply(
        ...
        tx_pos=tx_pos,
        rx_pos=rx_pos,
        tx_targets_m=sites.index_select(0, site_rank(batch)),
        rx_targets_m=sites.index_select(0, site_rank(batch)),
...
def test_applying_the_stage_twice_is_refused():
    ...
    once = stage.apply(
        ...
        tx_pos=radar.tx_pos,
        rx_pos=radar.rx_pos,
        tx_targets_m=_unit_sites(radar).index_select(0, site_rank(batch)),
        rx_targets_m=_unit_sites(radar).index_select(0, site_rank(batch)),
    )
...
def test_a_pattern_that_is_not_a_pattern_is_refused():
    ...
    join = types.SimpleNamespace(
        ...
        response_slot=torch.arange(stage.site_count, device=radar.device, dtype=torch.int64).repeat(
            stage.num_tx * stage.num_rx
        ),
    )
