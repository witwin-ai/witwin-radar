...
def _chain(noise, *, seed: int, lna=None, block_size: int = 256):
    """A chain at unit impedance, so the port stage scales nothing away.

    ``lna`` is a voltage gain in dB, ``seed`` the one Philox base every stage
    derives its own stream from, and ``block_size`` the launch width.
    """

    from witwin.radar.frontend import FrontendChain, FrontendSpec

    return FrontendChain(FrontendSpec(impedance=1.0, noise=noise, lna=lna, seed=seed), block_size=block_size)
...
def test_the_realisation_does_not_depend_on_the_launch_configuration(block):
    """The Philox-versus-per-thread-state test, run at two launch widths.
    ...
    resized = _chain(_thermal_and_phase(), seed=11, block_size=block).apply(signal)
...
def test_an_unusable_block_size_is_refused_rather_than_rounded():
    for value in (0, 17, 2048, -64):
        with pytest.raises(ValueError, match="power of two"):
            _chain(_thermal_only(), seed=1, block_size=value)
