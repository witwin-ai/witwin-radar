...
def _run(order):
    ...
    tau_in, c_in = fx.payload(len(INBOUND_ROWS), seed=61)
    tau_out, c_out = fx.payload(len(OUTBOUND_ROWS), seed=62)
    _, site_value = fx.payload(len(SITES), seed=63)
    ...
    inbound = fx.leg_batch(take(tau_in).to(torch.float32), take(c_in).to(torch.complex64))
    outbound = fx.leg_batch(tau_out.to(torch.float32), c_out.to(torch.complex64))
...
def test_a_permuted_publication_order_composes_to_an_identical_frame():
    ...
    for name in ("total_delay_s", "complex_transfer_ref"):
