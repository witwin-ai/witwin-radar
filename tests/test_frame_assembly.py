"""Structural packing from the synthesis pair axis to the processing frame cube.
...
from witwin.radar.paths import validate_pair_ordering
from witwin.radar.synthesis.assembly import PAIR_RANK_LAYOUT, assemble_frame_cube, pair_tx_index
...
def test_the_transmitter_table_inverts_the_pair_rank():
    ...
    for tx in range(NUM_TX):
        for rx in range(NUM_RX):
            assert int(tx_index[_pair_rank(tx, rx)]) == tx
