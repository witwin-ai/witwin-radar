...
import pytest
import torch
from support import aspect_oracle as oracle
...
def test_the_composed_round_trip_carries_the_row_response():
    ...
    dir_in = _directions(composer.inbound_row_count, seed=41, into_site=True)
    dir_out = _directions(composer.outbound_row_count, seed=42)
    tau_in, c_in = fx.payload(composer.inbound_row_count, seed=11)
    tau_out, c_out = fx.payload(composer.outbound_row_count, seed=12)
...
def test_the_aspect_rate_guard_refuses_a_fast_aspect_change():
    ...
    The join publishes no rate for the response's argument, so an aspect
    phase that walks is simply dropped, and no output reports that it was.
    """
