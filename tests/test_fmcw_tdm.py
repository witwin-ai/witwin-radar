...
* at ``num_tx > 1`` the per-TX phase walk must be REAL, so that the phase
  ``processing/angle.py::tdm_compensate`` removes downstream is the phase the
  kernel put there rather than a downstream reinterpretation;
* at ``num_tx = 1`` the slot must collapse to the chirp index EXACTLY.
...
import pytest
import torch
from support import fd
from support import reference_chain as ref
from support.dsp_ledger import DspLedger
from support.kernel_rows import beat_weight
...
def _frozen_channel_weight() -> complex:
    return beat_weight(FC_HZ, TAU_RT_S)
...
def test_a_second_transmitter_is_exactly_one_chirp_period_later(sample):
    ...
    tau, rate, weight, offsets, tx_index = _two_transmitters(_frozen_channel_weight())
    cube = synthesize_fmcw_rows(tau, rate, weight, offsets, spec, segment_tx_index=tx_index).cpu()
    ...
    still = synthesize_fmcw_rows(tau, torch.zeros_like(rate), weight, offsets, spec, segment_tx_index=tx_index).cpu()
...
def test_the_transmitter_table_is_what_decides_the_slot_not_the_pair_rank():
    ...
    spec = _spec(num_tx=2, num_rx=1)
    tau, rate, weight, offsets, tx_index = _two_transmitters(_frozen_channel_weight())
    forward = synthesize_fmcw_rows(tau, rate, weight, offsets, spec, segment_tx_index=tx_index)
    reversed_table = torch.tensor([1, 0], dtype=torch.int32, device="cuda")
    swapped = synthesize_fmcw_rows(tau, rate, weight, offsets, spec, segment_tx_index=reversed_table)
...
@pytest.mark.parametrize("sample", [0, NUM_SAMPLES - 1])
def test_the_downstream_tdm_compensation_removes_exactly_the_carrier_slot_phase(sample):
    """Cross-check against the consumer that has to undo this phase.

    ``processing/angle.py::tdm_compensate`` multiplies virtual antenna
    ...
    assert compensation == pytest.approx(2.0 * math.pi * FC_HZ * TAU_RATE * CHIRP_PERIOD_S, rel=1e-12)

    corrected = cube[:, 1, sample].to(torch.complex128) * complex(math.cos(-compensation), math.sin(-compensation))
    residual = float(torch.angle(corrected * torch.conj(cube[:, 0, sample].to(torch.complex128))).mean())
    t_m = sample / SAMPLE_RATE_HZ
    ramp_residual = 2.0 * math.pi * TAU_RATE * CHIRP_PERIOD_S * SLOPE_HZ_PER_S * (T_START_S - TAU_RT_S + t_m)
...
def test_a_pure_carrier_slot_phase_is_compensated_to_nothing():
    """With no ramp, the downstream compensation is exact - down to 1e-4 rad.
    ...
    assert raw == pytest.approx(2.0 * math.pi * FC_HZ * TAU_RATE * CHIRP_PERIOD_S, rel=1e-6)

    wavelength = C0 / FC_HZ
    radial_speed = C0 * TAU_RATE / 2.0
    compensation = 4.0 * math.pi * radial_speed * CHIRP_PERIOD_S / wavelength
    corrected = cube[:, 1, 0].to(torch.complex128) * complex(math.cos(-compensation), math.sin(-compensation))
    residual = float(torch.angle(corrected * torch.conj(cube[:, 0, 0].to(torch.complex128))).mean())
    assert abs(residual) < 1e-4


# --------------------------------------------------------------------------
# The same statement at a GENERAL delay rate and every transmitter
# --------------------------------------------------------------------------
...
def test_tdm_phase_matches_downstream_compensation(num_tx):
    ...
    * a bistatic ``tau_rate`` that no monostatic ``v`` produced. ``tdm_compensate``
    ...
    spec = _spec(num_samples=4, slope_hz_per_s=0.0, t_start_s=0.0, num_tx=num_tx, num_rx=1)
    tau, rate, weight, offsets, tx_index = _n_transmitters(_frozen_channel_weight(), TAU_RATE_BISTATIC, num_tx)
    cube = synthesize_fmcw_rows(tau, rate, weight, offsets, spec, segment_tx_index=tx_index).cpu()
    ...
    for tx_i in range(num_tx):
        contract_phase = 2.0 * math.pi * FC_HZ * TAU_RATE_BISTATIC * CHIRP_PERIOD_S * tx_i
        compensation_phase = 4.0 * math.pi * radial_speed * tx_i * CHIRP_PERIOD_S / wavelength
        assert abs(compensation_phase - contract_phase) < 1e-4

        measured = float(torch.angle(cube[:, tx_i, 0].to(torch.complex128) * torch.conj(reference)).mean())
        ...
        corrected = cube[:, tx_i, 0].to(torch.complex128) * complex(
            math.cos(-compensation_phase), math.sin(-compensation_phase)
        )
    ...
    assert walks[0] < 1e-9
    assert all(later > earlier + 0.5 for earlier, later in zip(walks[:-1], walks[1:], strict=True))


# --------------------------------------------------------------------------
# The same statement through the PRODUCTION slot table
# --------------------------------------------------------------------------
...
def _identical_row_per_pair(*, num_tx: int, num_rx: int, rate: float):
    ...
    return SynthesisPathBatch(
        ...
        total_delay_s=torch.full((pairs,), TAU_RT_S, dtype=torch.float32, device="cuda"),
        delay_rate=torch.full((pairs,), rate, dtype=torch.float32, device="cuda"),
        complex_transfer_ref=torch.full((pairs,), weight, dtype=torch.complex64, device="cuda"),
        reference_frequency_hz=FC_HZ,
...
def test_the_production_slot_table_survives_the_downstream_compensation(num_tx):
    ...
    The VELOCITY sign is negated at the call. ``tdm_compensate`` takes the
    canonical closing-positive velocity, and ``C0 * tau_rate / 2`` is its
    negative; the compensation is the same phase either way.

    The ramp is switched off so the compensation is exact; its residual on a
    real chirp is measured by
    ``test_the_downstream_tdm_compensation_removes_exactly_the_carrier_slot_phase``.
    """
    ...
    num_rx = 2
    num_chirps = 4
    spec = _spec(num_samples=4, num_chirps=num_chirps, slope_hz_per_s=0.0, t_start_s=0.0, num_tx=num_tx, num_rx=num_rx)
    batch = _identical_row_per_pair(num_tx=num_tx, num_rx=num_rx, rate=TAU_RATE_BISTATIC)
    ...
    # The processing layout: virtual antenna va = tx * num_rx + rx, one column per
    # chirp, which is exactly what the AoA input carries per detection.
    aoa_input = frame[:, :, :, 0].reshape(num_tx * num_rx, num_chirps)
    radial_speed = C0 * TAU_RATE_BISTATIC / 2.0
    velocities = torch.full((num_chirps,), radial_speed, dtype=torch.float64, device=frame.device)
    ...
    axes = SimpleNamespace(slow_time_period_s=CHIRP_PERIOD_S * num_tx, num_tx=num_tx)

    raw = aoa_input.to(torch.complex128).cpu()
    compensated = tdm_compensate(aoa_input, -velocities, array, axes).to(torch.complex128).cpu()
    ...
    for tx_i in range(num_tx):
        for rx_i in range(num_rx):
            ...
            walks.append(abs(float(torch.angle(raw[va] * torch.conj(reference)).mean())))
            residual = float(torch.angle(compensated[va] * torch.conj(reference)).mean())
    ...
    assert all(later > earlier + 0.5 for earlier, later in zip(per_tx[:-1], per_tx[1:], strict=True))


# --------------------------------------------------------------------------
# The num_tx = 1 slot identity
# --------------------------------------------------------------------------
...
def _single_row():
    ...
    weight = torch.tensor([_frozen_channel_weight()], dtype=torch.complex64, device="cuda")
...
def test_a_single_transmitter_makes_the_slot_the_chirp_index_exactly():
    ...
    explicit = synthesize_fmcw_rows(
        tau, rate, weight, offsets, spec, segment_tx_index=torch.zeros(1, dtype=torch.int32, device="cuda")
    )
...
def test_the_slot_is_num_tx_chirp_periods_wide():
    ...
    widened = synthesize_fmcw_rows(tau, rate, weight, offsets, _spec(num_chirps=6, chirp_period_s=2.0 * CHIRP_PERIOD_S))
...
def test_the_transmitter_table_must_span_the_segments():
    ...
    with pytest.raises(ValueError, match="one transmitter index per sensor-pair"):
        synthesize_fmcw_rows(
            tau, rate, weight, offsets, spec, segment_tx_index=torch.zeros(5, dtype=torch.int32, device="cuda")
        )


# --------------------------------------------------------------------------
# AD across the slot axis
# --------------------------------------------------------------------------
...
def ad_target():
    ...
    return torch.randn((AD_SPEC.num_chirps, len(AD_OFFSETS) - 1, AD_SPEC.num_samples), dtype=torch.complex128)


def _oracle_loss(tau, rate, weight, target):
    iq = ref.beat_samples(tau, rate, weight, torch.tensor(AD_OFFSETS, dtype=torch.int64), AD_SPEC, AD_TX_INDEX)
...
def test_the_float64_oracle_knows_about_slot_time():
    ...
    weight = torch.tensor([_frozen_channel_weight()] * 2, dtype=torch.complex128)
    ...
    for sample in (0, AD_SPEC.num_samples - 1):
        walk = float(torch.angle(cube[:, 1, sample] * torch.conj(cube[:, 0, sample])).mean())
        t_m = sample * AD_SPEC.sample_period_s
        analytic = 2.0 * math.pi * TAU_RATE * CHIRP_PERIOD_S * (FC_HZ + SLOPE_HZ_PER_S * (T_START_S - TAU_RT_S + t_m))
        ...
        assert abs(float(torch.angle(single_tx[:, 1, sample] * torch.conj(single_tx[:, 0, sample])).mean())) < 1e-12
...
def test_native_vjp_matches_the_oracle_across_the_slot_axis(ad_target):
    ...
    iq = synthesize_fmcw_rows(tau, rate, weight, offsets, AD_SPEC, segment_tx_index=tx_index)
    ...
    for index in range(len(AD_DELAYS)):
        assert fd.relative_error(float(tau.grad[index]), float(o_tau.grad[index]), floor=1e-6) < 2e-3
        assert fd.relative_error(float(rate.grad[index]), float(o_rate.grad[index]), floor=1e-6) < 2e-3
        assert fd.relative_error(float(weight.grad[index].real), float(o_weight.grad[index].real), floor=1e-9) < 2e-3
        assert fd.relative_error(float(weight.grad[index].imag), float(o_weight.grad[index].imag), floor=1e-9) < 2e-3
...
def test_the_oracle_gradients_agree_with_float64_finite_differences(ad_target):
    ...
    for index in range(len(AD_DELAYS)):
        measured_tau = fd.central_difference(
            lambda value: _oracle_loss(value, rate.detach(), weight.detach(), ad_target), tau.detach(), index, 1e-14
        )
        assert fd.relative_error(measured_tau, float(tau.grad[index]), floor=1e-6) < 1e-4

        measured_rate = fd.central_difference(
            lambda value: _oracle_loss(tau.detach(), value, weight.detach(), ad_target), rate.detach(), index, 1e-14
        )
        assert fd.relative_error(measured_rate, float(rate.grad[index]), floor=1e-6) < 1e-4

        measured_weight = fd.central_difference(
            lambda value: _oracle_loss(tau.detach(), rate.detach(), value, ad_target), weight.detach(), index, 1e-6
        )
        assert fd.relative_error(measured_weight, float(weight.grad[index].real), floor=1e-9) < 1e-5
    ...
    for name, (direction, step) in directions.items():
        with forward_ad.dual_level():
            ...
            if name == "tau":
                ...
                dual_weight = forward_ad.make_dual(weight, torch.complex(tangent, torch.zeros_like(tangent)))
            else:
                dual_weight = forward_ad.make_dual(weight, torch.complex(torch.zeros_like(tangent), tangent))
            assert not dual_tau.requires_grad
            iq = synthesize_fmcw_rows(dual_tau, dual_rate, dual_weight, offsets, AD_SPEC, segment_tx_index=tx_index)
        ...
        def evaluate(scale: float, name=name, direction=direction):
            ...
            return ref.beat_samples(
                moved_tau, moved_rate, moved_weight, torch.tensor(AD_OFFSETS, dtype=torch.int64), AD_SPEC, AD_TX_INDEX
            )
        ...
        torch.testing.assert_close(
            jvp, expected, rtol=2e-3, atol=2e-3 * scale, msg=lambda text, name=name: f"{name}: {text}"
        )
...
FMCW_OPERATORS = ("fmcw_beat_forward", "fmcw_beat_backward", "fmcw_beat_jvp")
...
def test_a_real_tdm_frame_assembles_into_the_processing_layout(multi_endpoint_spike):
    ...
    packing into the rank-4 cube ``processing`` consumes. The empty pairs survive
...
def test_one_tdm_frame_is_one_launch_and_no_host_observation(multi_endpoint_spike, monkeypatch):
    ...
    frozen legs and are already pinned by ``test_join_host_budget``.
    """
    ...
    ledger = DspLedger(monkeypatch, FMCW_OPERATORS, operators)
    cube = fmcw.synthesize_fmcw(batch, spec)
    frame = assemble_frame_cube(cube, num_tx=spec.num_tx, num_rx=spec.num_rx)
    assert frame.shape[0] == spec.num_tx

    assert ledger.launches == {"fmcw_beat_forward": 1, "fmcw_beat_backward": 0, "fmcw_beat_jvp": 0}, ledger.launches
