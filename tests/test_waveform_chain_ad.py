...
**The supported forward-mode shape is one dual level covering all three
endpoint sets**: an inbound leg's tangent needs the transmitter and the site
while the outbound leg's needs the site and the receiver. The last section pins
it.
"""
...
# --------------------------------------------------------------------------
# The premise: these really are two other waveforms, on the real fixture
# --------------------------------------------------------------------------
...
def test_the_two_cubes_have_their_own_shapes_and_are_not_the_beat_cube(spike, specs):
    ...
    ofdm = wc.synthesize("ofdm", spike.frame(None, drv.make_response())[0], specs["ofdm"])
    pulsed = wc.synthesize("pulsed", spike.frame(None, drv.make_response())[0], specs["pulsed"])
...
def test_the_pulsed_fixture_uses_an_lfm_and_says_why(spike, specs):
    ...
    ``test_pulsed_ad.py``. A pulsed end-to-end AD test built on one would
...
# --------------------------------------------------------------------------
# Reverse mode against a fourth-order difference of the whole chain
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Forward mode, against the reverse gradient it must reproduce
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The structural zeros, which are answers rather than gaps
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The forward-mode leg-coverage boundary, recorded
# --------------------------------------------------------------------------
...
def test_one_dual_level_over_all_three_endpoint_sets_is_the_supported_shape(spike, specs):
    ...
    and receivers with a zero tangent when only the sites move - is the
    supported shape: an inbound leg's tangent needs the transmitter and the
    site while the outbound leg's needs the site and the receiver, so a round
    trip whose two ends both move cannot be expressed one tensor at a time.
    ...
    only that the call is ACCEPTED.
    """
