...
2. **slot-batched ``reevaluate_slots`` -> loss.** ``test_slot_batching.py``
...
# --------------------------------------------------------------------------
# Frontend -> loss, behind a real cube
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The noise realisation is reproducible under AD
# --------------------------------------------------------------------------
...
def _slot_loss(spike, values, *, ad_mode="none", kind="fmcw"):
    ...
    frames = spike.slot_frames(inbound, outbound, mx.response_of(values))
...
def test_a_slot_batched_replay_carries_the_single_frame_gradient_exactly(spike, values):
    ...
    pinned by ``test_slot_batching.py``; its DERIVATIVE was not pinned
...
def _pattern_loss(radar, stage, composed, sites: torch.Tensor) -> torch.Tensor:
    ...
    reason ``test_antenna_pattern_route.py`` states for not putting an
...
def test_a_sensor_weight_gradient_reaches_a_synthesized_waveform_cube(spike):
    ...
    ``test_sensor_weight.py`` validates the weight kernel's jvp against a
