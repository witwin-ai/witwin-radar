...
import check_native_bindings as gate
...
def test_a_contract_test_that_names_neither_the_symbol_nor_its_owner_fails(tmp_path):
    ...
    def mutate(data):
        ...
        entry["contract_test"] = "tests/test_native_diagnostics.py"
...
def test_no_facade_row_needs_the_contract_test_note(manifest):
    """Every contract test names its symbol, so no row holds the hatch."""

    noted = [entry["symbol"] for entry in manifest["operators"] if entry.get("contract_test_note") is not None]
    assert noted == [], noted
