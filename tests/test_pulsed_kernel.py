...
from support.pulsed_grid import (
    BANDWIDTH_HZ,
    C0,
    F_REF_HZ,
    NUM_SAMPLES,
...
# --------------------------------------------------------------------------
# The matched-filter peak location - the primary delay assertion
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Fractional-delay fidelity - the snapping guard
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Unit energy and the exact peak identity
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The rectangular pulse's matched-filter triangle
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Per-pulse Doppler advance, two-sided
# --------------------------------------------------------------------------
...
def _measured_pulse_slope(spec, sample: int, rate: float) -> float:
    ...
    return lsq_slope(unwrapped(cube[:, 0, sample]))
...
def test_both_carrier_homes_produce_the_same_slow_time_slope(sample):
    ...
    from_kernel = lsq_slope(unwrapped(kernel_cube[:, 0, sample]))
...
# --------------------------------------------------------------------------
# Dead rows, empty segments, zero rows, out-of-gate rows
# --------------------------------------------------------------------------
...
def test_one_pulsed_frame_is_one_launch_and_no_host_observation(multi_endpoint_spike, monkeypatch):
    ...
    ledger = DspLedger(monkeypatch, PULSED_OPERATORS, operators)
