...
The asymmetry this closes: OFDM (``test_ofdm_ad.py``) and pulsed
(``test_pulsed_ad.py``) each have a per-variable jvp test on the CUBE and
...
``test_fmcw_beat_ad.py`` FD-validates in float64 first. Differencing the
...
#: The PRODUCTION carrier placement, matching ``test_fmcw_beat_ad.py``:
...
# --------------------------------------------------------------------------
# The per-variable jvp, on the cube
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The rate derivative in closed form
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The rate tangent is genuinely reaching the kernel
# --------------------------------------------------------------------------
...
def test_a_rate_only_tangent_is_not_the_zero_tangent():
    ...
    with forward_ad.dual_level():
        ...
        first_chirp = float(tangent[0].abs().max())
