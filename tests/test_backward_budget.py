...
forward number. ``test_pipeline_budget.py`` times a primal pipeline,
``test_join_host_budget.py`` sweeps a primal slot loop, and the one backward
statement anywhere - ``test_synthesis_launch_budget.py``'s one-companion-launch
...
1. the two-way join's context bytes, PREDICTED from the fixture's own row
   counts rather than written down as a total, so it fails on a change to what
   the join saves;
...
from support.dsp_ledger import TWO_WAY_JOIN_TAPE_FORMULA, DspLedger, two_way_join_tape_bytes
...
def _cuda_ms(fn, *, warmup: int = 10, runs: int = 50) -> float:
    """``test_pipeline_budget.py``'s convention, not a second one."""
...
def _join_tape(spike):
    """``(contexts, total bytes, distinct-storage bytes)`` for one frame."""
    ...
    try:
        ...
        spike.frame(sites, drv.make_response(), ad_mode="vjp")
    finally:
...
def test_the_join_context_saves_the_predicted_bytes(narrow):
    """One context per frame, and its bytes as a LAW in the row counts.

    Predicted from the fixture's own row counts rather than written down as a
    measured constant, so this fails when the join changes WHAT it saves and
    does not have to be re-measured when the fixture changes SIZE.
    """
    ...
    contexts, total, distinct = _join_tape(narrow)
    assert contexts == 1
    assert total == per_context, (total, per_context)
    assert distinct == per_context, (distinct, per_context)


# ---------------------------------------------------------------------------
# One backward launch per forward launch, at every boundary
# ---------------------------------------------------------------------------
...
def test_each_boundary_costs_one_backward_launch_per_forward_launch(name):
    ...
    ``test_synthesis_launch_budget.py`` makes this statement for the three
    ...
    with DspLedger(operators=(forward, backward, jvp), table=operators) as ledger:
        loss = boundary.loss(leaf)
        loss.backward()

    assert ledger.launches[forward] == 1, ledger.launches
    assert ledger.launches[backward] == 1, ledger.launches
    assert ledger.launches[jvp] == 0, ledger.launches
...
def test_the_frontend_costs_one_backward_launch_per_forward_stage():
    ...
    leaf = boundary.leaf.detach().clone().requires_grad_(True)
    with DspLedger(operators=names, table=operators) as ledger:
        boundary.loss(leaf).backward()
    assert ledger.launches["frontend_noise_forward"] == 1, ledger.launches
    assert ledger.launches["frontend_noise_backward"] == 1, ledger.launches
    assert ledger.launches["frontend_agc_forward"] == 1, ledger.launches
    assert ledger.launches["frontend_agc_backward"] == 1, ledger.launches
    # The quantizer is on the far side of the wall and this chain has no ADC.
    assert ledger.launches["frontend_quantize_forward"] == 0, ledger.launches
...
# ---------------------------------------------------------------------------
# The full FMCW pipeline, backward
# ---------------------------------------------------------------------------
...
def test_the_full_fmcw_pipeline_backward_meets_its_time_budget(pipeline):
    ...
    assert median < PIPELINE_BACKWARD_BUDGET_MS, (
        f"fmcw pipeline backward: {median:.4f} ms exceeds the budget of {PIPELINE_BACKWARD_BUDGET_MS:.4f} ms "
        f"({BACKWARD_TIME_HEADROOM:.2f}x of {MEASURED_PIPELINE_BACKWARD_MS} ms)"
    )


def test_the_full_fmcw_pipeline_backward_meets_its_peak_memory_budget(pipeline):
    ...
    assert peak_mb < PIPELINE_BACKWARD_PEAK_BUDGET_MB, (
        f"fmcw pipeline backward peak: {peak_mb:.4f} MB (budget {PIPELINE_BACKWARD_PEAK_BUDGET_MB:.4f} MB)"
    )
...
# ---------------------------------------------------------------------------
# The Channel inner loop, as Radar runs it
# ---------------------------------------------------------------------------
...
def test_the_channel_reevaluate_reverse_pass_is_a_surcharge_not_a_second_solve(pipeline):
    ...
    report = (
        f"channel reevaluate: forward {forward_ms:.4f} ms, forward+backward {reverse_ms:.4f} ms, "
        f"ratio {ratio:.4f} (budget {REEVALUATE_VJP_RATIO_BUDGET:.2f}, "
        f"measured range {MEASURED_REEVALUATE_VJP_RATIO_RANGE})"
    )
    assert ratio > 1.0, f"{report}: a reverse pass that cost no more than the forward means the backward is not running"
    assert ratio < REEVALUATE_VJP_RATIO_BUDGET, (
        f"{report}: the reverse pass is no longer a surcharge on the frozen topology"
    )
...
# ---------------------------------------------------------------------------
# The ledger document says what this module measures
# ---------------------------------------------------------------------------
...
#: fixture-derived join law, so no enforcement was weakened, but the document
...
def test_the_ledger_bytes_formula_is_the_law_this_module_measures(narrow):
    ...
    assert _ledger_row("two-way join")[3].strip("`") == TWO_WAY_JOIN_TAPE_FORMULA
    contexts, total, _ = _join_tape(narrow)
    measured_per_context = total // contexts

    documented = two_way_join_tape_bytes(
    ...
    # The ledger's own quoted evaluation of that formula, checked rather than
    # trusted.
    small = _ledger_row("two-way join")[4]
    assert small.startswith(f"{two_way_join_tape_bytes(R_in=2, R_out=2, S=2, K=2)} B"), small
