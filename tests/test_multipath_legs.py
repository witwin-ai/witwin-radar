...
Every analytic expectation comes from ``support.single_site_geometry`` in float64,
...
def _moving_frame(spike):
    """A four-row multipath frame whose rows carry the closed-form Doppler rate.

    The composer publishes no rate; the frozen-weight synthesis kernel takes
    one as an input, so the fixture's float64 image-source rate for
    ``SITE_VELOCITY_M_PER_S`` is written onto each row by its component key.
    """

    tx, site, rx = drv.positions()
    composed, _, _ = spike.paths(tx, site, rx, drv.make_response())
    shifts = geo.combined_doppler_hz()
    rate = torch.tensor(
        [
            -shifts[_combined_key(spike, composed, row)] / geo.REFERENCE_FREQUENCY_HZ
            for row in range(composed.path_count)
        ],
        dtype=torch.float32,
        device="cuda",
    )
    return composed.total_delay_s.clone(), rate, composed.complex_transfer_ref.clone(), composed
