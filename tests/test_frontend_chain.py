...
def test_the_quantization_error_variance_is_the_step_squared_over_twelve():
    ...
    expected = quantization_variance(bits=10, full_scale=1.0)
    assert math.isclose(float(error.real.double().var(unbiased=False)), expected, rel_tol=1e-2)
    assert math.isclose(float(error.imag.double().var(unbiased=False)), expected, rel_tol=1e-2)
...
def test_the_full_scale_sine_sqnr_matches_the_textbook_figure():
    ...
    assert abs(measured - full_scale_sine_sqnr_db(10)) < 0.3


# ---------------------------------------------------------------------------
# Phase-noise PSD asymptote
# ---------------------------------------------------------------------------
...
def test_the_phase_noise_spectrum_follows_the_free_running_asymptote():
    ...
    for target in (offset / 4, offset / 2, offset, 2 * offset, 4 * offset):
        ...
        predicted_dbc = single_sideband_dbc_per_hz(
            innovation_sigma_rad=expected_sigma, sample_rate_hz=sample_rate, offset_hz=float(frequencies[index])
        )
        predicted = 10.0 ** (predicted_dbc / 10.0)
...
# ---------------------------------------------------------------------------
# AGC breaks linearity, and the physics tests know it
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# The AGC costs no host observation
# ---------------------------------------------------------------------------
...
def test_the_quantizer_refuses_a_differentiable_input():
    """Fail loud, naming the non-differentiability of ``round``.
    ...
    with pytest.raises(RuntimeError, match="straight-through surrogate"):
