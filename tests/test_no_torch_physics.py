...
#: The physics owner packages. ``processing`` is NOT here: it is the
#: allowlisted Torch/DSP exception.
...
#: named by the function it sits in. Both are FREEZE-TIME SETUP:
#: ``_place_antennas`` and ``_world_from_local_matrix`` orthonormalise a pose
#: once per radar, and a ``Radar`` is immutable, so a new pose is a new radar
#: rather than a second run of the same setup. Neither is a per-path hot path.
#:
...
class _UnownedWaveform:
    """The smallest thing that can name a waveform kind with no owner."""

    kind = "chirp_sequence_from_the_future"


def test_an_unknown_waveform_kind_is_refused_at_construction():
    """The one check: the stored discriminator is validated where it is stored."""

    from witwin.radar.radar import RadarSystemConfig

    with pytest.raises(ValueError, match="waveform.kind must be one of"):
        RadarSystemConfig(waveform=_UnownedWaveform(), sensors=None, propagation=None)
...
#: ``torch.device`` and ``torch.cuda.is_available`` belong to the device
#: admission (``resolve_device``): a descriptor and a predicate, still no tensor.
