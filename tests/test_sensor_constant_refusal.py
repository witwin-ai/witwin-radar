...
The refusal is at CONSTRUCTION - before ``validate``, before a plan, before
any launch - and it is ``witwin.radar.policy.refuse_derivative``'s wording with
the sensor family's own reason. Almost everything here is deliberately
CPU-only: the refusal is a property of the contract and asking for a GPU to
check it would have made it a test people skip. The last section is the
exception, and it earns its ``--gpu`` mark - it drives the refusal through the
geometry a REAL production frame builds, the antenna-pattern stage's.
...
* the six differentiable inputs still work and are covered by
  ``test_sensor_weight.py``, whose gradient tests would fail if this
...
from witwin.radar.sensors import ROW_KIND_VIA, SensorWeightGeometry, SensorWeightPlan

NUM_TX = 2
NUM_RX = 2
ROWS = 3

#: The five frozen float fields on the geometry, with the shape each one has.
...
# --------------------------------------------------------------------------
# Every frozen field refuses, in both AD modes
# --------------------------------------------------------------------------
...
def test_a_marked_frozen_geometry_field_is_refused(field):
    ...
    assert "SensorWeightGeometry" in message and f"{field} carries a gradient" in message
...
def test_a_dual_carrying_frozen_geometry_field_is_refused(field):
    ...
    with forward_ad.dual_level():
        ...
        with pytest.raises(RuntimeError) as excinfo:
            SensorWeightGeometry(**fields)
    assert "SensorWeightGeometry" in str(excinfo.value) and f"{field} carries a forward tangent" in str(excinfo.value)
...
def test_a_marked_pattern_table_is_refused(index):
    ...
    covered, in both modes, by ``test_sensor_weight.py``. Marking the
    ...
    with pytest.raises(RuntimeError) as excinfo:
        _plan(tuple(tables))
    assert "SensorWeightPlan" in str(excinfo.value) and f"tables[{index}] carries" in str(excinfo.value)


# --------------------------------------------------------------------------
# It is a refusal: nothing is produced
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The boundaries: what is deliberately NOT refused
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The refusal from the real production entry point
# --------------------------------------------------------------------------
#
# The production consumer is ``sensors.py``. Its geometry is a STAGE-OWNED
# constant: the velocities, the fixed length and the normals are zeros the
# stage allocates at freeze time; no caller can hand any of them a marked
# value. The section therefore asserts both halves - that the production
# geometry carries nothing marked, and that the SAME geometry with one field
# marked is still refused.
...
def test_a_marked_field_on_the_production_geometry_is_still_refused():
    ...
    closest a caller can get to a marked frozen input, and it has to fail
    before any launch rather than run a frame and return ``None``.
    """
    ...
    assert "SensorWeightGeometry" in str(excinfo.value) and "site_velocity" in str(excinfo.value)
...
def test_the_production_route_still_carries_the_position_gradient():
    ...
    weight = stage.apply(
        ...
        tx_pos=radar.tx_pos,
        rx_pos=radar.rx_pos,
        tx_targets_m=sites.index_select(0, site_rank(composed)),
        rx_targets_m=sites.index_select(0, site_rank(composed)),
