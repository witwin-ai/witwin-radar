...
* The row-position ordering is reached by ``test_two_way_join_identity.py``,
  which permutes fabricated leg rows on purpose, and again by
  ``test_identity_key_columns.py``.
* The identity-key columns are reached ONLY by
  ``test_identity_key_columns.py``. Every OTHER fabricated leg is built
...
from reference.two_way_torch import PerSiteResponse
...
def test_the_multi_site_delay_gradient_is_the_closed_form_one():
    ...
    VJP is value-checked at two sites in ``test_two_way_join_ad.py``
...
def test_freeze_host_reads_are_counted_at_multi_pair_width(spike, monkeypatch):
    ...
    assert ledger.host["tolist"] == 14, ledger.host
