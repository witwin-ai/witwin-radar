...
def _aspect_rows(spike, sites=None, transmitters=None, *, ad_mode: str = "none"):
    ...
    rows = aspect_response().evaluate_rows(spike.composer, inbound, outbound, _flags(spike))
...
# --------------------------------------------------------------------------
# The Channel-side claim, observed from Radar
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Reverse mode, against finite differences
# --------------------------------------------------------------------------
...
def test_the_aspect_gradient_is_exactly_zero_when_field_direction_is_detached(spike):
    ...
    real, imaginary = aspect_response().evaluate_rows(spike.composer, severed[0], severed[1], _flags(spike))
    w_re, w_im = weights
...
# --------------------------------------------------------------------------
# All the way to a synthesized cube
# --------------------------------------------------------------------------


def _cube_loss(spike, spec, sites, *, ad_mode: str = "none"):
    composed, _, _ = spike.frame(sites, aspect_response(), ad_mode=ad_mode)
...
def test_the_direction_term_is_load_bearing_in_the_synthesized_loss(spike, spec):
    ...
    def gradient(*, sever: bool) -> torch.Tensor:
        ...
        composed = spike.composer.compose(inbound, outbound, aspect_response())
        cube = synthesize_fmcw(to_synthesis(composed), spec)
