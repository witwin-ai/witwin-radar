...
from support.kernel_rows import channel_coefficient, cuda_rows, lsq_slope, synthesis_batch, unwrapped
from support.pulsed_grid import stored
...
def _frozen_channel_weight(amplitude: complex = 1.0 + 0.0j, tau: float = TAU_RT_S):
    return channel_coefficient(F_REF_HZ, tau, amplitude)


def _cube(spec, delays, weights, rates, offsets=None) -> torch.Tensor:
    tau, rate, transfer, table = cuda_rows(delays, weights, rates, offsets)
    return synthesize_cfr_rows(tau, rate, transfer, table, spec).cpu().to(torch.complex128)


# --------------------------------------------------------------------------
# The subcarrier phase slope - the primary delay assertion
# --------------------------------------------------------------------------
...
def test_the_subcarrier_phase_slope_reports_the_round_trip_delay_in_seconds():
    ...
    analytic_step = -2.0 * math.pi * DF_HZ * stored(TAU_RT_S)
    assert analytic_step == pytest.approx(-0.018611103798605002, rel=1e-9)
    assert spec.subcarrier_phase_step_rad(stored(TAU_RT_S)) == pytest.approx(analytic_step, rel=1e-12)
...
def test_the_delay_is_round_trip_and_is_never_doubled():
    ...
    A synthesis kernel that formed ``tau = 2 d / c0`` from a delay that is
    already the round trip would report exactly twice the range,
    self-consistently, at every geometry
    and with no other symptom.
    """

    spec = _spec()
    single = lsq_slope(unwrapped(_cube(spec, [TAU_RT_S], [_frozen_channel_weight()], [0.0])[0, 0]))
    doubled = lsq_slope(
        unwrapped(_cube(spec, [2.0 * TAU_RT_S], [_frozen_channel_weight(tau=2.0 * TAU_RT_S)], [0.0])[0, 0])
    )
...
# --------------------------------------------------------------------------
# The CIR peak, as a secondary check
# --------------------------------------------------------------------------
...
def _measured_symbol_slope(spec, subcarrier: int, rate: float) -> float:
    ...
    return lsq_slope(unwrapped(cube[:, 0, subcarrier]))
...
def test_the_slow_time_slope_carries_the_whole_carrier_not_just_the_subcarrier():
    ...
    analytic = _analytic_symbol_slope(spec, 0, stored(TAU_RATE))
    ...
    assert _subcarrier_only_slope(spec, 0, stored(TAU_RATE)) == 0.0
    ...
    understatement = _analytic_symbol_slope(spec, top, stored(TAU_RATE)) / _subcarrier_only_slope(
        spec, top, stored(TAU_RATE)
    )
...
def test_the_slow_time_slope_carries_the_subcarrier_dependent_correction():
    ...
    analytic = _subcarrier_only_slope(spec, top, stored(TAU_RATE))
...
def test_both_carrier_homes_produce_the_same_slow_time_slope(subcarrier):
    ...
    from_kernel = lsq_slope(unwrapped(_cube(kernel_owned, [TAU_RT_S], [1.0 + 0.0j], [TAU_RATE])[:, 0, subcarrier]))
    analytic = _analytic_symbol_slope(production, subcarrier, stored(TAU_RATE))
...
def test_a_speed_past_the_unambiguous_bound_aliases():
    ...
    rate = 2.0 * speed / C0
    true_slope = _analytic_symbol_slope(spec, 0, stored(rate))
    ...
    assert measured == pytest.approx(_analytic_symbol_slope(spec, 0, stored(inside_rate)), rel=1e-5)


# --------------------------------------------------------------------------
# The cyclic prefix, through the production entry point
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Dead rows, empty segments, zero rows
# --------------------------------------------------------------------------


def _cuda_batch(**layout):
    return synthesis_batch(
        reference_frequency_hz=F_REF_HZ, tau_s=TAU_RT_S, tau_rate=TAU_RATE, weight=_frozen_channel_weight(), **layout
    )
...
def test_the_carrier_rate_multiplies_the_drift_and_the_subcarrier_the_full_delay():
    ...
    * the carrier-rate phase is exactly absent, because it multiplies the drift.
    ...
    assert step == pytest.approx(-2.0 * math.pi * DF_HZ * stored(TAU_RT_S), abs=2.0e-6)
...
def test_one_ofdm_frame_is_one_launch_and_no_host_observation(multi_endpoint_spike, monkeypatch):
    ...
    are already pinned by ``test_join_host_budget``.
    """
    ...
    ledger = DspLedger(monkeypatch, OFDM_OPERATORS, operators)
