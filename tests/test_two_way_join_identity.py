...
def _run(inbound_order, outbound_order, *, requires_grad=False):
    ...
    tau_in, c_in = fx.payload(len(inbound_rows), seed=31)
    tau_out, c_out = fx.payload(len(outbound_rows), seed=32)
    _, site_value = fx.payload(len(SITES), seed=33)
    ...
    inbound = fx.leg_batch(leaves["tau_in"], leaves["c_in"], row_valid=take(valid_in, inbound_order))
    outbound = fx.leg_batch(leaves["tau_out"], leaves["c_out"], row_valid=take(valid_out, outbound_order))
