...
import pytest
import torch
from reference.two_way_torch import PerSiteResponse, join_reference
from support import join_fixture as fx
...
def _frame(composer, *, valid=None, seed=(11, 12)):
    rows_in = composer.inbound_row_count
    rows_out = composer.outbound_row_count
    tau_in, c_in = fx.payload(rows_in, seed=seed[0])
    tau_out, c_out = fx.payload(rows_out, seed=seed[1])
    valid_in, valid_out = valid if valid is not None else (None, None)
    inbound = fx.leg_batch(tau_in.float(), c_in.to(torch.complex64), row_valid=valid_in)
    outbound = fx.leg_batch(tau_out.float(), c_out.to(torch.complex64), row_valid=valid_out)
    _, site_value = fx.payload(composer.site_count, seed=99)
...
def test_the_native_primal_matches_the_retained_torch_composition():
    ...
    tau, transfer = _reference(composer, inbound, outbound, response, None)
...
def test_a_dead_row_publishes_exactly_zero_in_every_output():
    ...
    tau, transfer = _reference(composer, inbound, outbound, response, expected)
    torch.testing.assert_close(composed.total_delay_s, tau, rtol=1e-6, atol=0.0)
    scale = float(transfer.abs().max())
    torch.testing.assert_close(composed.complex_transfer_ref, transfer, rtol=1e-5, atol=scale * 1e-6)
