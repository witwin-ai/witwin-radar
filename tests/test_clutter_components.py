...
from support import clutter_scenes as cs
...
from witwin.radar.processing import combine_incoherent
from witwin.radar.synthesis import select_component
...
def test_a_deeper_leg_becomes_its_own_class(spike, index):
    ...
    proves the depth predicate is live rather than dead configuration.
    """
...
def test_the_component_cubes_sum_to_the_full_cube(spike, index):
    ...
    assert residual <= atol, (
        f"coherent recombination residual: {residual:.6e} absolute, "
...
def test_a_component_export_costs_one_synthesis_and_no_host_observation(spike, index, monkeypatch):
    ...
    are declared. The second is structural: exporting ``n`` components costs
    ``n`` synthesis launches against the one an unseparated frame pays, and an
    empty component is not free.
    """
...
def test_the_target_only_cube_matches_the_float64_oracle(spike, index):
    ...
    assert error / scale < 1.0e-5, f"target-only cube vs float64 oracle: {error / scale:.3e} relative"
...
def test_incoherent_combination_adds_power_and_says_so(spike, index):
    ...
    assert cross > 1.0e-3 * float(power.max()), f"incoherent vs coherent cross term: {cross:.6e}"
...
def test_incoherent_combination_of_disjoint_delays_is_the_magnitude_sum(spike, index):
    ...
    assert relative <= 2.0 / math.sqrt(ratio) * (1.0 + 1.0e-3), f"dominant-bin relative cross term: {relative:.3e}"
    ...
    assert difference > 1.0e-3 * float(power.max()), f"overlapping-bin cross term: {difference:.6e}"
