...
import witwin.radar.processing.range_doppler as md
...
def _peak_hz(sequence: torch.Tensor) -> float:
    ...
    return float(frequencies[spectrum.abs().argmax(dim=-1)])
...
#: simulation is native, post-processing is Torch. ``processing`` already carries
...
def test_microdoppler_analysis_is_torch_only():
    ...
    # stripped first, deliberately: ``pulsed_echo.cu`` names
    # ``processing/signal.py`` to say where the processing that consumes its
    # output lives. That reference is the split being documented, which is the
    # opposite of the split being broken, and a scan that failed on it would
    # push the documentation out of the kernels.
...
def test_the_analysis_module_is_importable_without_cuda_being_resolved():
    ...
    # The original module object goes back afterwards: a second copy left in
    # sys.modules would give later tests a RangeProfile class that the rest of
    # the package does not recognise.
    original = sys.modules.pop("witwin.radar.processing.range_doppler", None)
    try:
        ...
        if original is not None:
            sys.modules["witwin.radar.processing.range_doppler"] = original
