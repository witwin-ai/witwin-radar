...
@pytest.mark.parametrize("sample", [0, NUM_SAMPLES - 1])
def test_the_sigproc_tdm_compensation_removes_exactly_the_carrier_slot_phase(sample):
...
def test_tdm_phase_matches_downstream_compensation(num_tx):
    ...
    * a bistatic ``tau_rate`` that no monostatic ``v`` produced. ``sigproc``
    ...
    for tx_i in range(num_tx):
        ...
        sigproc_phase = 4.0 * math.pi * radial_speed * tx_i * CHIRP_PERIOD_S / wavelength
        ...
        corrected = cube[:, tx_i, 0].to(torch.complex128) * complex(math.cos(-sigproc_phase), math.sin(-sigproc_phase))
...
class _FrameLedger:
    ...
    def __init__(self, monkeypatch, operators) -> None:
        self.launches = dict.fromkeys(("fmcw_beat_forward", "fmcw_beat_backward", "fmcw_beat_jvp"), 0)
