...
runtime = _load("radar_cuda_probe.runtime", os.path.join(cuda_dir, "runtime.py"))
package.runtime = runtime

result = {}
try:
    module = runtime.build_extension()
...
def _prebuilt_in(cuda_dir: Path) -> Path:
    return cuda_dir / "prebuilt" / runtime.prebuilt_extension_path().name
...
def _rewrite_record(binary: Path, **changes: object) -> None:
    ...
    record_path = runtime.build_info_sidecar_path(binary)
    record = json.loads(record_path.read_text(encoding="utf-8"))
    record.update(changes)
    record["build_fingerprint"] = runtime.compute_build_fingerprint(record)
    runtime.write_sidecars(binary, record)
...
def test_the_packaged_prebuilt_is_the_normal_load_source(packaged_binary):
    ...
    assert result["radar_abi_version"] == runtime.RADAR_ABI_VERSION
...
def test_a_missing_prebuilt_raises_and_does_not_compile(tmp_path, packaged_binary):
    ...
    assert result["cpp_extension_imported"] is False
    assert not list(scratch_temp.glob(f"{runtime.EXTENSION_NAME}*")), sorted(
...
def test_a_binary_without_its_record_is_refused(tmp_path, packaged_binary):
    cuda_dir = _copy_cuda_tree(tmp_path)
    runtime.build_info_sidecar_path(_prebuilt_in(cuda_dir)).unlink()
...
def test_a_binary_without_its_fingerprint_sidecar_is_refused(tmp_path, packaged_binary):
    cuda_dir = _copy_cuda_tree(tmp_path)
    runtime.fingerprint_sidecar_path(_prebuilt_in(cuda_dir)).unlink()
...
def test_a_corrupted_fingerprint_sidecar_is_refused(tmp_path, packaged_binary):
    cuda_dir = _copy_cuda_tree(tmp_path)
    sidecar = runtime.fingerprint_sidecar_path(_prebuilt_in(cuda_dir))
...
def test_a_record_from_another_abi_version_is_refused(tmp_path, packaged_binary):
    cuda_dir = _copy_cuda_tree(tmp_path)
    _rewrite_record(_prebuilt_in(cuda_dir), radar_abi_version=runtime.RADAR_ABI_VERSION + 1)
...
def test_a_record_with_an_unknown_field_is_refused(tmp_path, packaged_binary):
    ...
    cuda_dir = _copy_cuda_tree(tmp_path)
    record_path = runtime.build_info_sidecar_path(_prebuilt_in(cuda_dir))
    record = json.loads(record_path.read_text(encoding="utf-8"))
...
def test_a_complete_developer_override_loads(tmp_path, packaged_binary):
    ...
    for source in (
        developer_binary,
        runtime.build_info_sidecar_path(developer_binary),
        runtime.fingerprint_sidecar_path(developer_binary),
    ):
        shutil.move(str(source), str(packaged_copy / source.name))
    fingerprint = runtime.fingerprint_sidecar_path(moved).read_text(encoding="ascii").strip()
...
reached = []
runtime._jit_build_extension = lambda **kwargs: reached.append(kwargs) or "JIT"
try:
    returned = runtime.build_extension()
...
def test_preparing_build_tools_without_asking_for_a_build_raises():
    ...
    with pytest.raises(runtime.RadarExtensionLoadError, match="WITWIN_RADAR_NATIVE_BUILD"):
        runtime._ensure_windows_build_tools_on_path()
    with pytest.raises(runtime.RadarExtensionLoadError, match="WITWIN_RADAR_NATIVE_BUILD"):
        runtime._ensure_cuda_home_from_nvcc()
...
def test_the_recorded_symbol_set_matches_the_binding_manifest(packaged_binary):
    ...
    record = runtime.read_build_info(packaged_binary)
    manifest = json.loads((REPO_ROOT / "ci" / "native-binding-manifest.json").read_text(encoding="utf-8"))
...
def test_the_packaged_record_validates_against_this_checkout(packaged_binary):
    info = runtime.validate_identity(packaged_binary, runtime.extension_sources())
    assert info["extension_name"] == runtime.EXTENSION_NAME
    assert info["radar_abi_version"] == runtime.RADAR_ABI_VERSION
    assert info["build_type"] in runtime.BUILD_TYPES
    assert info["torch_target_version"] == runtime.TORCH_TARGET_VERSION
    assert info["source_fingerprint"].startswith(runtime.source_fingerprint())
