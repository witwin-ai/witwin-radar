...
def _frozen_channel_weight(amplitude: complex = 1.0 + 0.0j) -> complex:
    return beat_weight(FC_HZ, TAU_RT_S, amplitude)
...
def test_the_beat_tone_reports_the_round_trip_delay_in_seconds():
    ...
    assert exact == pytest.approx(SLOPE_HZ_PER_S * stored(TAU_RT_S), rel=1e-8)
...
def test_the_delay_is_round_trip_and_is_never_doubled():
    ...
    assert single == pytest.approx(SLOPE_HZ_PER_S * stored(short_delay), rel=1e-8)
    assert doubled == pytest.approx(SLOPE_HZ_PER_S * stored(2.0 * short_delay), rel=1e-8)
    assert doubled == pytest.approx(2.0 * single, rel=1e-6)


# --------------------------------------------------------------------------
# On-grid peak magnitude
# --------------------------------------------------------------------------
...
def test_the_unambiguous_speed_bound_is_owned_by_the_fmcw_spec():
    ...
    from support import single_site_geometry as single
