"""TDM slot phase, end to end on the scene-driven route.

This is the only place the two halves of TDM meet:
...
``test_fmcw_tdm.py`` pins the slot table and the kernel's own arithmetic;
...
from conftest import STANDARD_CONFIG, scene_radar, simulate_point_targets
...
def _radar():
    return scene_radar(STANDARD_CONFIG)
