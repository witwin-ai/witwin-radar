...
``test_no_torch_physics.py`` already established, plus one runtime
...
2. **The fence, over the physics owners.** ``tests/processing/test_processing_owners.py``
   ...
   criterion: the frozen primitive list itself is forbidden in every physics
   and synthesis owner, so a primitive added to the list is forbidden there
...
# ---------------------------------------------------------------------------
# The frozen vendor DSP primitive list
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# The fence, and the allowance lists
# ---------------------------------------------------------------------------
...
def test_no_synthesis_or_physics_module_calls_a_frozen_dsp_primitive():
    ...
    ``test_processing_owners`` scans the whole tree outside the facade for a fixed list of
...
# ---------------------------------------------------------------------------
# No Radar processing field in the Channel capability record
# ---------------------------------------------------------------------------
...
def test_the_native_binding_manifest_carries_no_processing_symbol():
    ...
    by ``test_native_binding_manifest.py`` - so what is asserted here is the
