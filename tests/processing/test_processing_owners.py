...
#: Production modules outside ``processing/`` allowed to name a fenced DSP call.
#: Empty: no simulation owner reads a spectrum. ``ci/check_torch_physics_allowlist.py``
#: asserts this set equals ``fence_allowances`` in ``ci/torch-physics-allowlist.json``,
#: so an allowance is added in both places or in neither.
FENCE_ALLOWANCES = {}
...
def _code(source: str) -> str:
    ...
    which is the same reason ``test_no_torch_physics`` strips its
...
def test_the_hard_coded_half_wavelength_spacing_is_gone():
...
def test_no_numpy_survives_in_the_processing_facade():
...
def test_the_tdm_compensation_has_no_python_transmitter_loop():
...
def test_the_whole_chain_runs_from_a_synthesis_layout_to_a_detection_frame():
    ...
    detected = ca_cfar(beams.data.abs(), guard_cells=(1, 2), training_cells=(2, 3), pfa=1e-2)
    ...
    cells = ca_cfar(combined.abs(), guard_cells=(1, 2), training_cells=(2, 3), pfa=1e-2)
    cloud = point_cloud(cells, rd, array, max_points=8)
