...
def test_single_definition_gate_rejects_shared_owner_and_target(
    tmp_path: Path,
) -> None:
    ...
    _json(
        tmp_path / "ci" / "architecture-manifest.json",
        {
            ...
            "concept_owners": {
                "configuration": "witwin.radar.radar",
                "session": "witwin.radar.radar",
            },
        },
    )
    _json(
        tmp_path / "ci" / "public-api-manifest.json",
        {
            "modules": {
                "witwin.radar": {
                    ...
                    "RadarAlias": "witwin.radar.radar.Radar",
                }
            }
        },
    )
...
def test_public_api_manifest_gate_rejects_duplicate_target() -> None:
    ...
    assert errors == [
        "target witwin.radar.radar.Radar exposed twice: "
        "witwin.radar.Radar and witwin.radar.RadarAlias"
    ]
...
def test_public_api_manifest_gate_handles_declared_value_exports(monkeypatch) -> None:
    ...
    module = SimpleNamespace(
        __all__=("ISOTROPIC_PATTERN",),
        ISOTROPIC_PATTERN=object(),
    )
    monkeypatch.setattr(gate.importlib, "import_module", lambda _name: module)
    manifest = {
        "modules": {
            "witwin.radar.sensors": {
                "ISOTROPIC_PATTERN": "witwin.radar.sensors.ISOTROPIC_PATTERN",
            }
        },
        "root_class_members": {},
...
def test_release_claim_gate_rejects_retired_policy_and_false_success(
    tmp_path: Path,
) -> None:
    ...
    _write(
        tmp_path / "docs" / "dev" / "plans" / "phase10-deferred-release-matrix.md",
        "manylinux_2_28\n",
    )
    _write(
...
def test_governance_inventory_gate_rejects_open_and_unproven_rows(
    tmp_path: Path,
) -> None:
    ...
    for index in range(1, 28):
        ...
        rows.append(
            f"| GOV-{index:03d} | debt | owner | G | gate | scope | "
            f"{status} | {evidence} |\n"
        )
    path = (
        tmp_path
    ...
    assert gate.audit(tmp_path, require_closed=False) == [
        "GOV-002 is closed without evidence"
    ]
