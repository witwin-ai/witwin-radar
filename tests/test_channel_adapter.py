...
from support import single_site_geometry as geo
from support import single_site_world as world
from witwin.channel.propagation import consumer

from witwin.radar.channel import ChannelPropagationAdapter
from witwin.radar.propagation import RadarEndpointSpec, require_endpoint_role
...
def test_reevaluate_publishes_radar_leg_aliasing_consumer_storage(adapter, compiled_scene, frozen_inbound):
    ...
    legs = adapter.reevaluate_slots(frozen_inbound, sources, sinks, slot_count=1, ad_mode="none")
    ...
    # hand back its storage.
    again = adapter.reevaluate_slots(frozen_inbound, sources, sinks, slot_count=1, ad_mode="none")
...
def test_reevaluate_aliases_by_object_identity(adapter, frozen_inbound, monkeypatch):
    ...
    legs = adapter.reevaluate_slots(frozen_inbound, _tx(), _site_sink(), slot_count=1, ad_mode="none")
...
def test_freeze_is_never_called_per_frame(adapter, monkeypatch):
    ...
    for frame in range(5):
        ...
        adapter.reevaluate_slots(
            inbound, _tx(), world.endpoint_spec(site, geo.SITE_STABLE_ID), slot_count=1, ad_mode="none"
        )
        adapter.reevaluate_slots(
            outbound,
...
def test_differentiable_power_is_rejected_before_any_native_work(adapter, frozen_inbound):
    ...
    with pytest.raises(NotImplementedError, match="primal-only"):
        adapter.reevaluate_slots(frozen_inbound, sources, _site_sink(), slot_count=1, ad_mode="vjp")
...
def test_differentiable_polarization_is_rejected(adapter, frozen_inbound):
    ...
    with pytest.raises(NotImplementedError, match="primal-only"):
        adapter.reevaluate_slots(frozen_inbound, _tx(), sinks, slot_count=1, ad_mode="vjp")
...
def test_frequency_mismatch_is_refused_with_the_contract_error(compiled_scene, frozen_inbound):
    ...
    with pytest.raises(ValueError, match="reference_frequency_hz does not exactly match"):
        mismatched.reevaluate_slots(frozen_inbound, _tx(), _site_sink(), slot_count=1, ad_mode="none")
    ...
    legs = survivor.reevaluate_slots(frozen_inbound, _tx(), _site_sink(), slot_count=1, ad_mode="none")
...
def test_adapter_rejects_unknown_ad_mode(adapter, frozen_inbound):
    with pytest.raises(NotImplementedError, match="unsupported ad_mode"):
        adapter.reevaluate_slots(frozen_inbound, _tx(), _site_sink(), slot_count=1, ad_mode="reverse")
...
def test_endpoint_role_contract_matches_channel(adapter, frozen_inbound):
    ...
    with pytest.raises(ValueError, match="source endpoint requires powers_w"):
        adapter.reevaluate_slots(frozen_inbound, _site_sink(), _site_sink(), slot_count=1, ad_mode="none")
    with pytest.raises(ValueError, match="sink endpoint must not carry powers_w"):
        adapter.reevaluate_slots(frozen_inbound, _tx(), _site_source(), slot_count=1, ad_mode="none")
