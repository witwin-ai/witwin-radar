...
def _legs(delays, coefficients, *, valid=None, device="cpu"):
    rows = len(delays)
    ...
    return RadarLegBatch(
        ...
        field_direction=zeros(torch.float32, (rows, 3)),
...
def test_delay_is_additive_and_transfer_factorizes():
    ...
    inbound = _legs([1.0e-8], [0.5 + 0.25j], device="cuda")
    outbound = _legs([2.0e-8], [-0.125 + 0.75j], device="cuda")
