...
from witwin.radar.synthesis.fmcw import _synthesize_fmcw_observations
...
def test_ragged_observations_match_complex_oracle_and_ad(carrier):
    ...
    actual = _synthesize_fmcw_observations(delay, weight, offsets, clock, spec)
    tau = delay.double()
...
def test_empty_observations_and_time_derivative_refusal():
    ...
    result = _synthesize_fmcw_observations(delay, weight, offsets, delay, spec)
    assert torch.equal(result, torch.zeros_like(result))
    with pytest.raises(RuntimeError, match="ADC times"):
        _synthesize_fmcw_observations(delay, weight, offsets, delay.clone().requires_grad_(), spec)
