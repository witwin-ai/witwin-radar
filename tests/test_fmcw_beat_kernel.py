...
Everything here is checked against a closed form computed in float64, never
...
from witwin.radar.synthesis.fmcw import channel_phasor_to_beat_weight, synthesize_fmcw_rows
...
def test_the_conjugation_sign_is_anchored_to_a_hand_computed_sample():
    ...
    of both would agree with itself.
...
def test_loading_the_extension_is_free_and_side_effect_free_after_the_first_call():
    ...
    first = runtime.build_extension()
    path_length = len(os.environ.get("PATH", ""))
    for _ in range(8):
        assert runtime.build_extension() is first
