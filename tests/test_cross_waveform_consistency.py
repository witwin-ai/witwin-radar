...
from support.kernel_rows import lsq_slope, unwrapped
from support.multi_endpoint_specs import (
    C0,
    F_REF_HZ,
    FMCW_DELAY_RESOLUTION_S,
    FMCW_SAMPLES,
    OFDM_SUBCARRIERS,
    PULSED_DELAY_RESOLUTION_S,
    fmcw_spec,
    ofdm_spec,
    pulsed_spec,
)
...
def ofdm_delay_s(row_response: torch.Tensor, spec: OfdmSpec) -> float:
    ...
    slope = lsq_slope(unwrapped(values))
...
# ---------------------------------------------------------------------------
# I1 - One delay, three estimators
# ---------------------------------------------------------------------------


def test_all_three_estimators_return_the_same_round_trip_delay(frame):
...
def test_the_three_waveforms_carry_one_doppler_with_two_signs(spike):
    ...
    composed, _, _ = spike.frame()
    # The composer publishes no rate; the frozen-weight kernels take one as an
    # input, so the float64 closed form for these velocities is written onto
    # the rows in the join's canonical order.
    rate = torch.tensor(
        geo.combined_delay_rate_s_per_s(spike.predicted_combined_rows(), velocities), dtype=torch.float32, device="cuda"
    )
    batch = drv.to_synthesis(replace(composed, delay_rate=rate))
...
# ---------------------------------------------------------------------------
# Absolute level against the bistatic radar equation (criterion A3)
# ---------------------------------------------------------------------------
