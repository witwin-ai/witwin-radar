...
def _parameters(composer, *, device: str = "cuda"):
    ...
    _, site = fx.payload(composer.site_count, seed=103, device=device)
    return {"tau_in": tau_in, "tau_out": tau_out, "c_in": c_in, "c_out": c_out, "response": site}
...
def _oracle_loss(composer, values, weights, row_valid):
    tau_rt, transfer = join_reference(
...
def _production_batch(composer, base, *, row_valid=None, requires_grad=False):
    ...
    inbound = fx.leg_batch(live["tau_in"], live["c_in"], row_valid=valid_in)
    outbound = fx.leg_batch(live["tau_out"], live["c_out"], row_valid=valid_out)
