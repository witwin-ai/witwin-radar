...
def test_path_amplitudes_respect_antenna_pattern_gain():
    radar = _radar_with_pattern(
        {
            "kind": "separable",
            "x_angles_deg": [0, 45, 90],
    ...
    total = torch.tensor([[[4.0]]], dtype=torch.float32)
    center_sample = normalize_interpolated_sample(
        (
    ...
    off_axis_sample = normalize_interpolated_sample(
        (
            torch.tensor([1.0], dtype=torch.float32),
            torch.tensor([[off_axis, 0.0, -off_axis]], dtype=torch.float32),
        ),
        device=radar.device,
    )

    center_peak = compute_path_amplitudes(radar, center_sample, total).abs().max()
    off_axis_peak = compute_path_amplitudes(radar, off_axis_sample, total).abs().max()
...
def test_path_amplitudes_respect_simplified_polarization_gain():
    ...
    hh_peak = compute_path_amplitudes(radar_hh, sample, total).abs().max()
    hv_peak = compute_path_amplitudes(radar_hv, sample, total).abs().max()
...
def test_polarization_requires_surface_normals():
    ...
    with pytest.raises(ValueError, match="surface normals"):
        compute_path_amplitudes(radar, sample, total)


def test_reference_mimo_supports_pattern_with_multi_tx_config():
    radar = Radar(
        RadarConfig.from_dict({
            "num_tx": 3,
            "num_rx": 4,
    ...
    frame = pytorch_mimo_from_samples(radar, collect_interpolated_samples(radar, interp))
    assert frame.shape == (3, 4, 2, 16)
