...
the ``sqrt(4 pi sigma) / lambda`` law and against a measured ``d(loss)/d(amplitude)``, which is
...
from witwin.radar.scattering import SPEED_OF_LIGHT_M_PER_S, ScalarRcsResponse, rcs_amplitude
...
def _loss(spike, spec, response) -> torch.Tensor:
    ...
    composed, _, _ = spike.frame(None, response, ad_mode="none")
...
# --------------------------------------------------------------------------
# The law, and the host-float path that must not move
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Reverse mode against three independent oracles
# --------------------------------------------------------------------------
...
def test_the_gradient_is_the_amplitude_gradient_through_the_square_root(spike, spec, reverse):
    ...
    (``test_single_site_end_to_end.py``), so this also states that the new leaf
...
# --------------------------------------------------------------------------
# Forward mode, and the two modes against each other
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# The refusals that keep the new route honest
# --------------------------------------------------------------------------
