...
``Function`` in one of the seven owner files. Its limit, stated honestly: a read
...
#: The seven files that own a ``torch.autograd.Function``. Between them they
#: hold the eleven owners ``test_the_context_scan_is_not_vacuous`` counts.
...
# ---------------------------------------------------------------------------
# Nobody outside an owner reads a context
# ---------------------------------------------------------------------------
...
def test_the_context_scan_is_not_vacuous():
    ...
    Eleven owners each read their tape in backward and jvp: twenty-two reads,
    including refreshed ADC observations and adaptive path interpolation. A scanner that
    ...
    assert total == 22, {name: len(reads) for name, reads in live.items()}
...
# ---------------------------------------------------------------------------
# No public result carries a tape
# ---------------------------------------------------------------------------
...
def frame():
    ...
    composed, inbound, outbound = spike.frame(sites, drv.make_response(), ad_mode="vjp")
...
@pytest.mark.gpu
def test_the_synthesis_and_sensor_results_carry_no_tape(frame, monkeypatch):
    ...
    monkeypatch.setattr(SensorWeightResult, "from_components", capture)
...
def test_the_diagnostics_alias_the_frame_rather_than_a_detached_copy(simulated):
    """The four ``last_*`` members are NOT detached.

    Detaching them would cost a copy of every payload tensor and would make the
    one diagnostic a caller reaches for while debugging a gradient the one that
    cannot carry it. What is forbidden is a FIELD holding the tape, which the
    walk above checks; holding a ``grad_fn`` is not that, and a caller who
    wants the graph released drops the result.
    """
