...
def test_a_fully_populated_config_sends_exactly_the_allowed_keywords(monkeypatch):
    ...
    system_config = _populated_radar().system_config
    recorder = _Recorder()
    ...
    adapter.reevaluate_slots(frozen, sources, sinks, slot_count=1, ad_mode="none")
...
def test_the_adapter_constructor_accepts_only_the_propagation_block():
    ...
    assert parameters == {"compiled_scene", "reference_frequency_hz", "components", "max_depth"}, sorted(parameters)


# ---------------------------------------------------------------------------
# OFDM does not multiply the reference frequency
# ---------------------------------------------------------------------------
...
def _synthesis_batch(radar, *, rows: int = 3):
    """A composed batch, built directly rather than through a solve.
    ...
    with the provenance every Channel-composed batch carries.
    """
