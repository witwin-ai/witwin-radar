...
def _inbound(spike, sites=None, *, ad_mode="none"):
    return spike.adapter.reevaluate_slots(
        spike.inbound, _transmitter_spec(spike), _site_spec(spike, sites), slot_count=1, ad_mode=ad_mode
    )
...
def test_a_stale_compiled_scene_never_answers(monkeypatch):
    ...
    baseline = DspLedger(monkeypatch, ())
    _inbound(spike)
    assert baseline.host["item"] == 1, baseline.host
    ...
    counter = DspLedger(monkeypatch, ())
    with pytest.raises(ValueError, match="geometry_version"):
        on_late.reevaluate_slots(
            spike.inbound, _transmitter_spec(spike), _site_spec(spike), slot_count=1, ad_mode="none"
        )
    assert counter.host_observation_count == 0, counter.host
...
# --------------------------------------------------------------------------
# A row stopped existing
# --------------------------------------------------------------------------


# --------------------------------------------------------------------------
# A row started existing
# --------------------------------------------------------------------------
...
def test_the_motion_event_cadence_costs_what_it_says(monkeypatch):
    ...
    * discovery is the expensive tier, so a motion event must cost strictly
    ...
    assert event_ms > replay_ms, (
        f"motion event (compile + refreeze + freeze x2 + composer): {event_ms:.3f} ms; "
        f"batched replay, {slots} slots, two legs: {replay_ms:.3f} ms ({replay_ms / slots:.4f} ms/slot)"
    )

    counter = DspLedger(monkeypatch, ())
    state.spike.slot_legs(stack, slot_count=slots)
    assert counter.host["item"] == 2, counter.host
    assert counter.host_observation_count == 2, counter.host
