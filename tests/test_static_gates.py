...
import check_raw_native_access
import check_test_oracle_isolation
import check_torch_physics_allowlist
...
def test_g4_fires_when_an_allowed_expression_gains_a_sibling(mirror: Path) -> None:
    ...
    target = mirror / "witwin" / "radar" / "radar.py"
    source = target.read_text(encoding="utf-8")
    original = "        if torch.linalg.norm(forward) <= 1e-12:"
    assert original in source
    target.write_text(
        source.replace(
            original,
            "        duplicate = torch.linalg.norm(forward)\n"
            "        if torch.linalg.norm(forward) * duplicate / duplicate <= 1e-12:",
            1,
        ),
        encoding="utf-8",
    )
...
def test_g4_fires_when_a_pytest_constant_drifts_from_the_record(mirror: Path) -> None:
    ...
    target = mirror / "tests" / "test_no_torch_physics.py"
    source = target.read_text(encoding="utf-8")
...
def test_g4_fires_when_the_fence_allowance_list_drifts(mirror: Path) -> None:
    ...
    target = mirror / "tests" / "processing" / "test_processing_owners.py"
    source = target.read_text(encoding="utf-8")
