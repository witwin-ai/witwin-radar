...
def _batch(rows: int, *, seed: int, row_valid: torch.Tensor | None = None):
    delay, coefficient = fx.payload(rows, seed=seed)
    return fx.leg_batch(
        delay.to(torch.float32).contiguous(), coefficient.to(torch.complex64).contiguous(), row_valid=row_valid
    )


def _response(composer, *, sites: int | None = None) -> PerSiteResponse:
    _, value = fx.payload(composer.site_count if sites is None else sites, seed=303)
...
def test_a_response_of_the_wrong_length_is_refused(sites):
    ...
    class _WrongLength:
        ...
        def evaluate(self, row_count, device):
            _, value = fx.payload(sites, seed=404, device=str(device))
