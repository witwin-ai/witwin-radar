...
``test_synthesis_contract``: the cyclic-prefix contract and the
...
# --------------------------------------------------------------------------
# The cyclic-prefix contract, fail-loud
# --------------------------------------------------------------------------
...
def test_there_is_no_clamping_path_in_the_ofdm_contract():
    ...
    # Two refusals, counted rather than described, so that a third has to be
    # added here deliberately: the foreign-spec TypeError and the cyclic-prefix
    # bound. It is a count of REFUSALS, and every one of them is all-or-nothing.
