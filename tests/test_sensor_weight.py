...
class PathSample:
    """The per-path tensors the oracle and the kernel rows are built from.

    ``tests/reference/path_math.py`` indexes it by attribute name.
    """
...
def _radar():
    ...
    An isotropic table is two knots at plus and minus 180 degrees with unit
    gain: the kernel-versus-Torch parity assertion would then hold on 1.0
    everywhere and the knot-crossing exclusion in the JVP test would have
    nothing left to exclude. The dipole is the pattern every number in this
    ...
    return Radar.from_dict(dict(STANDARD_CONFIG), pattern=Pattern.dipole())
...
def _sample(radar, count: int, *, seed: int = 0):
    ...
    points = world_from_local_points(radar, local.to(device))
    entry = world_from_local_points(radar, (local + 0.05 * torch.randn(count, 3, generator=generator)).to(device))
    return PathSample(
...
def _evaluate(radar, sample, *, velocities=None):
    ...
    return evaluate_sensor_weights(
        ...
        site_out=site_out,
        intensity=intensity,
        weight=torch.ones(intensity.shape[0], dtype=torch.complex64, device=radar.device),
        geometry=geometry,
        plan=_plan(radar),
    )
...
def _pattern_cell(radar, geometry, site_in, site_out) -> torch.Tensor:
    ...
    for vectors in (site_in - radar.tx_pos[geometry.tx_index], site_out - radar.rx_pos[geometry.rx_index]):
        local = local_from_world_vectors(radar, vectors)
        forward = -local[..., 2]
