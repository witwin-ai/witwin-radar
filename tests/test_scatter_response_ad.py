...
def _frame(composer, dir_in, dir_out):
    tau_in, c_in = fx.payload(composer.inbound_row_count, seed=21)
    tau_out, c_out = fx.payload(composer.outbound_row_count, seed=22)
...
def test_a_forward_tangent_on_the_aspect_axis_reaches_the_composed_transfer():
    ...
    _, c_in = fx.payload(composer.inbound_row_count, seed=21)
    _, c_out = fx.payload(composer.outbound_row_count, seed=22)
